...
public class SBTreeWALTest extends SBTreeTest {
  ...
  private void restoreDataFromWAL() throws IOException {
    ODiskWriteAheadLog log = new ODiskWriteAheadLog(4, -1, 10 * 1024L * OWALPage.PAGE_SIZE, null, actualStorage, 10, 256 * 1024);
        ...
        for (OWALRecord restoreRecord : atomicUnit) {
          ...
          if (restoreRecord instanceof OFileCreatedWALRecord) {
            ...
            if (!expectedWriteCache.exists(fileName))
              expectedReadCache.addFile(fileName, expectedWriteCache.externalFileId(fileCreatedCreatedRecord.getFileId()),
                  expectedWriteCache);
